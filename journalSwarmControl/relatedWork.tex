...
%%%%%%%Where should we add these?

%[1] requires robots that can execute task primitives.  Our particles simply move in the direction of the global control unless they are stopped by an obstacle. \cite{milutinovic2006modeling}
%[3] uses only Open-Loop control.  Our submission relies on state feedback \cite{palmer2011hamiltonian}
%[4] requires collective intelligence of a group of miniature robots\cite{prorok2011multi}
%[5] requires robots that can individually decide when to actuate and when to remain anchored, can perform onboard computation (generate a random number, calculate its bin and the next bin). \cite{demir2015probabilistic}
