...
\State $sign \gets 0$
\EndIf
\State Move by $(-r_1.x , -\min(r_2.y , r_1.y+ \Delta e_y)+\epsilon)$
\State Move by $(-\min(|-r_2.x+ \Delta e_x|, r_2.x), sign \, \min(\max(r_1.y + \Delta e_y, r_2.y)-\epsilon,L-\epsilon)+ \epsilon)$
\If {$r_2.y \neq r_1.y$}
\State Move by $(\max(\epsilon, \Delta e_x- (r_2.x - r_1.x)), -\min(r_1.y , r_2.y))$
\State Move by $\Delta e_x , (|sign|-1) \epsilon$
\EndIf
\State Move by $(e_2.x - r_2.x, e_2.y - r_2.y)$
\end{algorithmic}
\end{algorithm}
