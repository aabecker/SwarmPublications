...
To solve both these problems, this paper models object movement as a Markov Decision Process (MDP) with non-deterministic movement.  
  Value iteration,  as described by Thrun et al.~\cite{Thrun2005}, is used to learn an \emph{optimal policy}.
...
&\hat{V} (x_i) = \gamma \max_{\mathbf{u}} [r(x_i,\mathbf{u}) + \sum\limits_{j=1}^N \hat{V}(x_j) p(x_j | x_i,\mathbf{u})] \label{eq:ValueIteration} \nonumber\\
\text{end}&  %page 502
\end{align}
...
The interaction between the particles and object is impulsive so, like the study of impulsive pulling by Christensen  \cite{christensen2016let},  adding additional particles decreases completion time, but with diminishing returns. 
 After 75 particles, additional particles no longer can interact with the object and do not contribute to the task success. 
