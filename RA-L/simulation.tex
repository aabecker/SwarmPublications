...
Algorithms \ref{alg:PosControl2Robots} %\ref{alg:YControl}, 
was implemented in Mathematica using point robots (radius = $0$).  Fig.~\ref{fig:shapeControlMathematica1}  shows  an implementation of this algorithm. 
...
Random disturbances impair the performance of Alg. \ref{alg:PosControl2Robots} and Alg. \ref{alg:PosControlNRobots}. However, due to the central limit theorem, we can control the correlation of large numbers of particles. This section demonstrates controlling swarm correlation in simulation, using the 2D physics engine Box2D~\citep{catto2010box2d}.
 ...
 Without friction, covariance has minimal variation.  As friction increases, the correlation can be manipulated to greater degrees.
...
\end{figure}
 ...
 144 disc-shaped robots were also controlled by a closed-loop controller using the procedure in Sec. \ref{subsec:ClosedLoopCovarianceControl} with $c_1=0.1$. Fig. \ref{fig:CovVarControlPlot} illustrates that covariance and variances in $x$ and $y$ directions were controlled from a set of initial conditions.
\begin{figure}
\begin{center}
