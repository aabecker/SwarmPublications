...
Computing means, variances, covariance, and correlation requires integrating over the region $R$ containing the swarm:  %This is simplified using an indicator function $\bm{1}_A(x,y)$ that returns 1 if inside the region containing the swarm, and 0 else. 
...
\vspace{-1em}
\caption{Wall friction reduces the force for going forward $F_{\text{\emph{forward}}}$ on a robot near a wall, but not for a free robot. (Right) velocity of a fluid reduces to zero at the boundary.}
\label{fig:friction}
\end{center}
\end{figure} 
...
Online demonstration and source code at \citet{Shahrokhi2015mathematicaParticle}.
