...
The swarm must deliver the object to the goal region.  To solve this object manipulation task, we first discretize the environment. 
...
The robots were directed to assemble behind the object at  $\mathbf{b} - k_2 r_b \nabla \mathbf{M}(\mathbf{b})$, then move to  $\mathbf{b} - k_3 r_b \nabla \mathbf{M}(\mathbf{b})$ to push the object toward the goal location. However, BFS has no penalties on object collisions with obstacles, and the resulting collisions made the BFS solution slow.
 ...
 This paper applies value iteration algorithm mentioned in \cite{Thrun2005} to learn a policy for the Markov Decision Process $\mathbf{M}_{MDP}$. At each instance the object can move in eight directions or stay in place. The corresponding reward function gives a high reward to the goal location. All other free spaces have a small penalty; while obstacles have a large negative reward. Value iteration is used to learn an optimal policy which is a function that assigns a control input to every possible state.
...
where $u$ is action and $x$ is the current state. The policy iteration algorithm returns $\arg\max_u   [ r(x,u) + \sum\limits_{j=1}^N \hat{V}(x_j) p(x_j| u,x)]$. We set $N=200$ for the full convergence and $\hat{V}(x_j) $ is calculated by

\begin{align}
\hat{V} (x_j) = \gamma \max_u [r(x_j,u) + \sum\limits_{i=1}^N \hat{V}(x_i) p(x_i| u,x_j)],
\end{align}
where in our experiments $\gamma = 0.97$.
...
\end{overpic}
\vspace{-0.5em}
\caption{\label{fig:BFSGradient}The BFS algorithm finds the shortest path for the moveable object  to compute gradient vectors (left). Using policy iterations enables encoding penalties for being near obstacles (middle). At right, the results of policy iteration generated from the obstacles detected by the vision algorithm on the hardware experimental setup is shown.
%\vspace{-2em}
}
\end{figure}
...
\State Compute $\mathbf{M}$, the cost to goal    \Comment{using $\mathbf{M}_{BFS}$ or $\mathbf{M}_{MDP}$} 
