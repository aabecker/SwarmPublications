Sec. ection{Introduction}\label{sec:Intro}
...
\end{align}
Here $x_i$ and $y_i$ regard to the position and respectively $\dot{x}_i, \dot{y}_i$ are velocities of the particles. Our control problem is to design the control inputs $u_x(t), u_y(t)$ to make all $n$ particles achieve a task.
...
Sec.  \ref{sec:simulation} describes implementations of the algorithms in simulation and  Sec.  \ref{sec:expResults} describes hardware experiments with up to 100 robots, as shown in Fig.~\ref{fig:IntroPic}. After a review of recent related work in Sec.  \ref{sec:RelatedWork}, we end with directions for future research in Sec.  \ref{sec:conclusion}.
