...
However, process noise cancels the differentiating effects of inhomogeneity for large numbers of robots.  We desire control algorithms that extend to many thousands of robots.
...
Our $n$-robot system with 2 control inputs and 4$n$ states is inherently under-actuated, and superficially bears resemblance to compliant, under-actuated manipulators.  Our swarms conform to the object to be manipulated, but lack the restoring force provided by flexures in Odhner's work \cite{odhner2014compliant} or silicone in Deimel's work \cite{deimel2014novel}.  Our swarms tend to disperse and so to regroup them we require artificial forces like the variance control primitives in \S \ref{sec:VarianceControl}.
