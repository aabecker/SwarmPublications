...
%[1] requires robots that can execute task primitives.  Our particles simply move in the direction of the global control unless they are stopped by an obstacle.
%[3] uses only Open-Loop control.  Our submission relies on state feedback
%[4] requires collective intelligence of a group of miniature robots
%[5] requires robots that can individually decide when to actuate and when to remain anchored, can perform onboard computation (generate a random number, calculate its bin and the next bin).
