...
\begin{algorithmic}[1]
\Require knowledge of starting $(s_1,s_2)$ and ending $(e_1,e_2)$ positions of  two robots. 
$(0,0)$ is bottom corner, $s_1$ is topmost, leftmost robot, 
 $L$ is length of the walls. Current robot positions are $(r_1,r_2)$.
...
\State $sign \gets 0$
\EndIf
\State Move to $(-r_1.x , -\min(R_2.y , R_1.y+ \Delta e_y)+\epsilon)$
\State Move to $(-\min(|-r_2.x+ \Delta e_x|, r_2.x), sign \, \min(\max(r_1.y + \Delta e_y, r_2.y)-\epsilon,L-\epsilon)+ \epsilon)$
\If {$r_2.y \neq r_1.y$}
\State Move to $(\max(\epsilon, \Delta e_x- (r_2.x - r_1.x)), -\min(r_1.y , r_2.y))$
\State Move to $\Delta e_x , (|sign|-1) \epsilon$
\EndIf
\State Move to $(e_2.x - r_2.x, e_2.y - r_2.y)$
\end{algorithmic}
\end{algorithm}
