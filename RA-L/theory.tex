...
\vspace{-1em}
\caption{Pushing the swarm against a circular boundary wall allows limited control of the shape of the swarm, as a function of the fill level $h$ and the commanded movement direction $\beta$. Left plot shows locus of possible mean positions for four values of $h$. The locus of possible mean positions are concentric circles. See online interactive demonstration at \citep{Shahrokhi2015mathematicaParticle}.}
...
The range for the global input angle $\beta $ is [0,2$\pi $). In this range, the swarm assumes eight different polygonal shapes. The shapes alternate between triangles and trapezoids when the area $A$$<$1/2, and alternate between squares with one corner removed and trapezoids when $A$$>$1/2.

Computing the means, variances, covariance, and correlation requires integrating over the region $R$ containing the swarm:  %This is simplified using an indicator function $\bm{1}_A(x,y)$ that returns 1 if inside the region containing the swarm, and 0 else. 
...
\vspace{-1em}
\caption{Wall friction reduces the force for going forward $F_{\text{\emph{forward}}}$ on a robot near a wall, but not for a free robot. At left there is a fluid and inside it the velocity of the robot is almost zero at the surface and is more and more until it is the same as the free space on the threshold.}
\label{fig:friction}
\end{center}
\end{figure} 
...
{\begin{overpic}[width =\figwid]{S1_1.png}\put(55,15){$t$ = 0 s}
\end{overpic}
\begin{overpic}[width =\figwid]{S1_2.png}\put(55,15){$t$ = 0.14 s}
\end{overpic}
\begin{overpic}[width =\figwid]{S1_3.png}\put(55,15){$t$  = 0.29 s}
\end{overpic}
\begin{overpic}[width =\figwid]{S1_4.png}\put(55,15){$t$  = 0.45 s}
\end{overpic}
\begin{overpic}[width =\figwid]{S1_5.png}\put(55,15){$t$  = 0.7 s}
\end{overpic}}\\
