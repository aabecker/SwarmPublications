...
F_{forward}(y) &= F - F_f\begin{cases}  \frac{y-h}{h}   , &  y<h \label{eq:boundarylayerflow} \\
...
\State Move $(-\min(|-r_2.x+ \Delta e_x|, r_2.x), \sign(\Delta e_y) \, \min(\max(r_1.y + \Delta e_y, r_2.y)-\epsilon,L-r_2.y-\epsilon)+ \epsilon)$ \Comment{Adjust  $y$}
\If {$r_2.y \neq r_1.y$}
\State Move $(\max(\epsilon, -\Delta e_x+ (r_2.x - r_1.x)), -\min(r_1.y , r_2.y))$ \Comment{Touch bottom wall}
\State Move $(\Delta e_x , (|\sign(\Delta e_y)|-1) \epsilon)$ \Comment{Adjust $x$}
\EndIf
\If {$\Delta e_y \neq r_2.y - r_1.y$}
