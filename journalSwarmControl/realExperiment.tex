...
\begin{center}
	...
	\end{overpic}\\
	\begin{overpic}[width=0.35\columnwidth]{AllShapesR.pdf}%\put(5,85){b)}
	\end{overpic}
	\begin{overpic}[width=0.35\columnwidth]{PotentialExp.pdf}%\put(5,85){c)}
	\end{overpic}
...
(Top)Hardware platform:  table with 1.5$\times$1.2 m workspace, surrounded by four remotely triggered 30W LED and four 50W LED floodlights, with an overhead machine vision system. (Down left) A swarm of robots, all controlled by a uniform force field, can be effectively controlled by a hybrid controller that knows only the first and second moments of the robot distribution. Here is a swarm of hardware robots (kilobots) that pushes a green object with different shapes toward the goal. (Down right) The potential fields view in hardware experiments. See video attachment~\cite{ShivaVideo2015}
}
\end{figure*}
