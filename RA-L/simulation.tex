...
Two simulations were implemented using wall-friction for position control.  The first controls the position of two robots, the second controls the position of $n$ robots.  All code is available online at \citep{Zhao2016mathematica,Zhao2016mathematicaSquare}.
...
are only required if $|\Delta e_y - \Delta s_y|>L$. An online interactive demonstration and source code of the algorithm are available at \citep{Shahrokhi2015mathematicaParticle}.
...
Random disturbances impair the performance of Alg. \ref{alg:PosControl2Robots} and Alg. \ref{alg:PosControlNRobots}. Due to central limit theorem, we are able to control covariance of large swarms. This section demonstrates simulations of controlling covariance of the swarm.
These simulations use  the 2D physics engine Box2D, by \citep{catto2010box2d}.
