...
First, the robots are moved left away from the right wall, and down so all robots in $k^{th}$ row touch the bottom wall.
Second, a set of $\operatorname{DriftMove}$s are executed that move all robots in $k^{th}$ row left until $k$ touches the left wall, with no net movement of the other robots.
